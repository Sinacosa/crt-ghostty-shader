...
const float HORIZONTAL_LINE_STRENGTH = 0.14;
const float HORIZONTAL_LINE_SPACING = 14.0;
const float HORIZONTAL_LINE_WIDTH = 0.34;
const float HORIZONTAL_LINE_SOFTNESS = 0.10;
...
const float RADIAL_REFLECTION_STRENGTH = 0.075;
const float RADIAL_REFLECTION_RADIUS = 0.78;
const float RADIAL_REFLECTION_SOFTNESS = 2.4;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float scan = 1.0 - SCANLINE_STRENGTH * (0.5 + 0.5 * sin(warpedUv.y * paddedResolution.y * SCANLINE_DENSITY));
    float linePhase = fract(warpedUv.y * paddedResolution.y / HORIZONTAL_LINE_SPACING);
